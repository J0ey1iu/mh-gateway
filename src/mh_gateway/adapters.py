...
class SessionRepository(Protocol):
    ...
    # ── Detached run state (issue #63) ──
    # Multi-process deployments need the run status + cancel marker in the
    # shared store: the sessions list reads ``status`` from here and the
    # cancel endpoint writes ``cancel_requested`` here, so any worker can
    # see / stop a run regardless of which worker owns it.
    async def mark_run_started(self, session_id: str) -> None: ...
    async def mark_run_finished(self, session_id: str) -> None: ...
