...
async def _finalize_run(
    ...
    """
    try:
        # No agent-level stall watchdog: the lower layers own their timeouts
        # (LLM stream stall detection + per-tool timeouts) and their errors
        # propagate up to the agent, which then stops or retries.  A coarse
        # "no progress for N seconds" watchdog preempts that graceful
        # recovery (it force-cancelled runs whose tool was still inside its
        # own timeout).  A truly stuck run (deadlock) is stopped by the
        # explicit cancel endpoint instead.
        await task
