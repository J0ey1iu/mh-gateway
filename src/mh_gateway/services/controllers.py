"""Application-level looping controllers (goal / timer) for mh-gateway.

这些是应用层策略：judge LLM 语义、时间限制、轮数上限都是产品决策，
不属于 minimal-harness 框架能力。它们通过 SDK 公开的 ``Controller``
协议插入 runtime——这也是"外部应用在 controller 层插入自定义
controller"的活样本：框架只提供 ``Controller`` Protocol、
``ControllerRegistry`` 与 ``DefaultController`` 兜底。
